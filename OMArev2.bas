...
Sub SummaryStats()
    ...
    Dim pcbSum As Double, emSum As Double
    pcbSum = 0: emSum = 0
    
    Dim i As Long, qty As Variant
    
    ' New quantity column was added to the raw data, using that instead '
    For i = 2 To lastRow
        qty = ws.Cells(i, "K").Value
        If IsNumeric(qty) Then
            Select Case Trim(ws.Cells(i, "A").Value)
                Case "ASSEMBLY-PCB"
                    pcbSum = pcbSum + CDbl(qty)
                Case "ASSEMBLY-ELECTRO MECHANICAL"
                    emSum = emSum + CDbl(qty)
            End Select
        End If
    Next i
...
Sub ExportToTxt()
    ...
    pcbQty = CLng(ws.Range("M2").Value)
    emQty = CLng(ws.Range("N2").Value)
